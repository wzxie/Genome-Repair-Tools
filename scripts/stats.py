#!/usr/bin/env python3
"""
Chromosome Analysis Tool - Enhanced version
Optimized for multi-threaded telomere detection and memory usage
...
### CORE ANALYSIS FUNCTIONS (can be imported) ###
...
def detect_internal_telomeres(sequence, telomere_patterns, min_length=100, flank_size=5000):
    ...
    # Merge adjacent internal telomeres
...
def compile_telomere_patterns_for_plant():
    """Telomere patterns for plant genomes"""
    patterns = {
...
def analyze_single_chromosome_telomeres(args):
    """Multi-threaded function to analyze telomeres for a single chromosome"""
    ...
    # Internal telomeres - using simplified method
...
def analyze_fragments_optimized(sequence, gaps, min_fragment_length=None):
    """Analyze fragments between gaps with memory optimization"""
...
def create_spaced_diagram_to_file(f, seq_id, sequence_length, gaps, telomere_info, 
    ...
    """Create chromosome diagram and write to file (not print to screen)"""
...
### INDEPENDENT PROCESSING FUNCTIONS (can be pickled) ###
...
def process_single_sequence(record, telomere_data_dict, min_gap_size, analyze_fragments, 
    ...
    """Independent processing function that can be pickled for multiprocessing"""
    seq_id = record.id
    ...
    # Write gaps to file immediately to save memory
    if gaps:
    ...
    # Re-detect internal telomeres to ensure accuracy
    internal_telomeres = []
    if internal_patterns:
        ...
        # If internal_patterns is None, try to get from telomere_data_dict
    ...
    if analyze_fragments:
        ...
        # Write fragment info immediately
...
def process_single_sequence_wrapper(args):
    """Wrapper function to pack multiple parameters"""
...
### MAIN ANALYSIS FUNCTION - Can be imported and called directly ###
...
def analyze_chromosomes(
    ...
    verbose: bool = False  # control screen output
) -> dict:
    """
    Main analysis function - can be imported and called from other scripts
    
    Parameters:
        input_file: Path to input FASTA file
        detect_telomeres: Whether to detect telomeres
        analyze_fragments: Whether to analyze fragments
        save_diagrams: Whether to save diagrams to file
        min_gap_size: Minimum gap size to report
        min_fragment_length: Minimum fragment length to keep (bp) - set to None to keep all
        threads: Number of threads to use
        min_repeats: Minimum telomere repeats
        search_window: Telomere search window (bp)
        internal_flank: Internal telomere flank size (bp)
        diagram_width: Width of diagrams
        output_prefix: Prefix for output files
        memory_optimized: Use memory-optimized processing
        verbose: Whether to print detailed progress information
    
    Returns:
        dict: Analysis results including statistics and file paths
    """
    ...
    # Save original order of chromosome IDs
    original_order = [record.id for record in records]
    
    # Telomere detection (multi-threaded)
    ...
    if detect_telomeres:
        ...
        # Write telomere results to files - in original order
        ...
        # Even if not detecting telomeres, need to initialize internal_patterns for internal telomere detection
        internal_patterns = compile_telomere_patterns_for_plant()
    
    # Gap and fragment analysis
    if verbose:
    ...
    # Use dictionary to store results by seq_id
    results_by_seq_id = {}
    ...
    # Reorder results by original order
    for seq_id in original_order:
    ...
    # Save diagrams to file if requested
    if save_diagrams:
        if verbose:
    ...
    # Create filtered FASTA if needed
        ...
        with open(filtered_fasta, 'w') as f:
            for result in analysis_results:
                ...
                if fragment_info and 'kept_fragments_list' in fragment_info:
                    ...
                    # Concatenate kept fragments
                    new_sequence_parts = []
                    for i, (start, end, length, seq) in enumerate(kept_fragments):
                        if i > 0:
                            # Add 100 Ns gap between fragments
...
### COMMAND LINE INTERFACE - Only used when running as standalone script ###
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Basic usage - only specify input file
  python chromosome_analyzer.py -q genome.fasta
  
  # Detect telomeres and analyze fragments
  python chromosome_analyzer.py -q genome.fasta --detect-telomeres --analyze-fragments
  
        """
    )
    
    # Input file parameter (required)
    ...
    parser.add_argument('--min-fragment-length', type=int,
                       help='Minimum fragment length to keep (bp) - if not set, no filtering')
    ...
    try:
        ...
        results = analyze_chromosomes(
            input_file=args.query,  # using -q parameter value
            ...
            verbose=args.verbose or not args.quiet  # control screen output
        )
        
        # Show brief summary only in non-verbose mode and quiet mode
...
### New: Simplified command line interface, just input filename to run ###

if __name__ == "__main__":
    # Simplified version: directly accept filename as argument
    # Usage: python stats.py genome.fasta
    if len(sys.argv) == 2 and not sys.argv[1].startswith('-') and not sys.argv[1].startswith('--'):
        input_file = sys.argv[1]
        print("="*70)
        print("CHROMOSOME ANALYSIS TOOL - SIMPLIFIED MODE")
        print("="*70)
        print(f"Analyzing file: {input_file}")
        print("Using default parameters: detect telomeres + analyze gaps + save diagrams (no filtering)")
        print("-"*70)
        
        # Call main analysis function, enable all required features
        results = analyze_chromosomes(
            input_file=input_file,
            detect_telomeres=True,      # detect telomeres
            analyze_fragments=True,      # analyze gaps and fragments
            save_diagrams=True,          # save diagrams
            min_gap_size=1,              # detect all gaps
            min_fragment_length=None,     # critical! no min fragment length = no filtering
            output_prefix="analysis",     # output file prefix
            threads=16,                   # use 16 threads
        ...
        for name, path in results['output_files'].items():
            if path and os.path.exists(path):
                # Beautify file name display
                display_name = {
                    'gap_coordinates': 'Gap coordinates',
                    'fragment_info': 'Fragment information',
                    'telomere_info': 'Telomere information',
                    'internal_telomeres': 'Internal telomeres',
                    'chromosome_diagrams': 'Chromosome diagrams',
                    'filtered_fasta': 'Filtered sequences'
                }.get(name, name)
        ...
        print(f"  Processing speed: {summary['processing_speed_mbp_sec']:.1f} Mbp/sec")
        
        # Show small fragment information (optional)
        if results['analysis_results']:
            small_fragments = []
            for result in results['analysis_results']:
                if result.get('fragment_info') and result['fragment_info'].get('discarded_fragments_list'):
                    # discarded_fragments_list here are fragments smaller than 1M
                    for frag in result['fragment_info']['discarded_fragments_list']:
            ...
            if small_fragments:
                print(f"\nSmall fragments (<1Mbp):")
                print(f"   Total: {len(small_fragments):,}")
                # Show first 3 as examples
                print("   Examples (first 3):")
                for frag in small_fragments[:3]:
                    print(f"     • {frag['chr']}: {frag['start']:,}-{frag['end']:,} ({frag['length']:,} bp)")
        ...
        # If there are other parameters (like -h, --help, etc.), use original command line interface
        main()

### SUBSCRIPT CALLING INTERFACE ###

def run_as_subscript():
    """
    Specialized function for being called as a subscript
    Can be called from other Python scripts:
    
    from chromosome_analyzer_subscript import run_as_subscript
    
    results = run_as_subscript(
        query="my_genome.fa",
    ...
    """
    # Parse sys.argv, mimicking command line parameters

...
def find_quality_filtered_contigs_in_directory(search_dir: str, logger: logging.Logger = None) -> Optional[str]:
    if not os.path.exists(search_dir):
        return None
    
    patterns = [
        os.path.join(search_dir, "*merged_quality_filtered*", "*.fasta"),
        os.path.join(search_dir, "*quality_filtered*", "*.fasta"),
        os.path.join(search_dir, "*quality*", "*.fasta"),
        os.path.join(search_dir, "*filtered*", "*.fasta"),
        os.path.join(search_dir, "*_merged_quality_filtered.fasta"),
        os.path.join(search_dir, "*_quality_filtered.fasta"),
        os.path.join(search_dir, "*_filtered.fasta"),
        os.path.join(search_dir, "filled_genome_merged_quality_filtered.fasta"),
    ]
    
    for pattern in patterns:
        matches = glob.glob(pattern)
        for match in matches:
            if os.path.exists(match) and os.path.getsize(match) > 1000:
                filename = os.path.basename(match).lower()
                if any(keyword in filename for keyword in ['filtered', 'quality', 'qv']):
                    file_size_mb = os.path.getsize(match) / (1024 * 1024)
                    if logger:
                        logger.debug(f"Found potential filtered file: {match} ({file_size_mb:.2f} MB)")
                    return match
...
def prepare_final_contigs_files(generated_contigs: List[str], external_contigs: List[str], 
                               ...
                               assembly_output_dir: str = None,
                               quality_filter: bool = True,
                               quality_min_length: int = 100000,
                               quality_min_qv: float = 30.0,
                               skip_craq: bool = False,
                               reads_files: List[str] = None,
                               skip_reassessment: bool = False,
                               skip_assembly: bool = False) -> Optional[str]:
    if skip_assembly and external_contigs:
        merged_dir = os.path.join(output_dir, "merged_contigs")
        os.makedirs(merged_dir, exist_ok=True)
        
        final_contigs_file = os.path.join(merged_dir, "final_contigs.fasta")
        
        if external_contigs and os.path.exists(external_contigs[0]):
            shutil.copy2(external_contigs[0], final_contigs_file)
            file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024)
            logger.info(f"Using external contigs file (no assembly): {os.path.basename(external_contigs[0])}")
            logger.info(f"  Size: {file_size_mb:.2f} MB")
            logger.info(f"  Quality assessment: SKIPPED (no reads files)")
    ...
    if skip_reassessment and os.path.exists(final_contigs_file):
        file_size = os.path.getsize(final_contigs_file)
        if file_size > 1000:
            file_size_mb = file_size / (1024 * 1024)
            logger.info(f"  Using existing final_contigs.fasta (skip reassessment)")
            logger.info(f"    File: {final_contigs_file}")
            logger.info(f"    Size: {file_size_mb:.2f} MB")
            logger.info(f"    Note: Quality assessment skipped as requested")
            return final_contigs_file
        else:
            logger.warning(f"  Existing final_contigs.fasta is too small ({file_size} bytes), will recreate")
    
    filtered_contigs_found = None
    
    if assembly_output_dir and quality_filter:
        logger.info(f"Searching for pre-existing quality-filtered contigs in: {assembly_output_dir}")
        filtered_contigs_found = find_quality_filtered_contigs_in_directory(assembly_output_dir, logger)
    
    if filtered_contigs_found:
        shutil.copy2(filtered_contigs_found, final_contigs_file)
        file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024)
        logger.info(f"  Using existing quality-filtered contigs file: {os.path.basename(filtered_contigs_found)}")
        logger.info(f"    Size: {file_size_mb:.2f} MB")
        logger.info(f"    Source: {filtered_contigs_found}")
        logger.info(f"    (Skipping re-merge and re-filter since assembly already provided filtered file)")
        
        if os.path.exists(final_contigs_file):
            file_size = os.path.getsize(final_contigs_file)
            file_size_mb = file_size / (1024 * 1024)
            status = "empty" if file_size < 100 else "valid"
            logger.info(f"  Final contigs file: {final_contigs_file} ({file_size_mb:.2f} MB, {status}, quality-filtered)")
            return final_contigs_file
        else:
            logger.warning("  Failed to copy filtered contigs file")
    
    raw_merged_file = os.path.join(merged_dir, "raw_merged_contigs.fasta")
    
    if len(all_contigs) == 0:
        logger.warning("No contigs files available, creating empty final_contigs.fasta")
        ...
        shutil.copy2(all_contigs[0], raw_merged_file)
        source_type = "external" if all_contigs[0] in external_contigs else "generated"
        logger.info(f"  Single {source_type} contigs file: {os.path.basename(all_contigs[0])}")
    else:
        ...
        if merge_fasta_files(all_contigs, raw_merged_file):
    ...
    shutil.copy2(raw_merged_file, final_contigs_file)
    
    if quality_filter and not filtered_contigs_found and os.path.exists(final_contigs_file) and os.path.getsize(final_contigs_file) > 100:
        logger.info("Applying quality assessment and filtering to merged contigs...")
        
        if MODULES.get('run_assembly_gap_fill') is None:
            logger.warning("  assembly_gap_fill module not available, skipping quality filter")
        else:
            try:
                shutil.copy2(final_contigs_file, final_contigs_file + '.bak')
                
                quality_filter_prefix = os.path.join(merged_dir, "quality_filtered")
                
                config_dict = {
                    'query_fasta': final_contigs_file,
                ...
                if reads_files:
                    hifi_files = [f for f in reads_files if 'hifi' in f.lower() or 'pacbio' in f.lower()]
                    ont_files = [f for f in reads_files if 'ont' in f.lower() or 'nanopore' in f.lower()]
                    clr_files = [f for f in reads_files if 'clr' in f.lower()]
                    
                    if hifi_files:
                        config_dict['hifi_files'] = hifi_files[:1]
                    elif ont_files:
                        config_dict['ont_files'] = ont_files[:1]
                    elif clr_files:
                        config_dict['clr_files'] = clr_files[:1]
                
                logger.info(f"  Running quality filter with min_length={quality_min_length:,}bp, min_qv={quality_min_qv:.1f}")
                
                fill_result = MODULES['run_assembly_gap_fill'](config_dict)
                
                if fill_result.get('status') == 'success' and fill_result.get('output_file'):
                    filtered_file = fill_result.get('output_file')
                    if os.path.exists(filtered_file):
                        shutil.copy2(filtered_file, final_contigs_file)
                        
                        raw_size = os.path.getsize(final_contigs_file + '.bak')
                        filtered_size = os.path.getsize(final_contigs_file)
                        if raw_size > 0:
                            reduction = ((raw_size - filtered_size) / raw_size * 100) if raw_size > 0 else 0
                            logger.info(f"  Quality filter applied successfully: {reduction:.1f}% reduction")
                        else:
                            logger.info(f"  Quality filter applied successfully")
                    else:
                        logger.warning("  Filtered file not found, using original merged file")
                        if os.path.exists(final_contigs_file + '.bak'):
                            shutil.copy2(final_contigs_file + '.bak', final_contigs_file)
                else:
                    logger.warning(f"  Quality filter failed: {fill_result.get('message', 'Unknown error')}")
                    logger.warning("  Using original merged file (no quality filter)")
                    if os.path.exists(final_contigs_file + '.bak'):
                        shutil.copy2(final_contigs_file + '.bak', final_contigs_file)
    ...
    if os.path.exists(final_contigs_file):
        ...
        file_size_mb = file_size / (1024 * 1024)
        status = "empty" if file_size < 100 else "valid"
        filter_status = "pre-filtered" if filtered_contigs_found else ("quality-filtered" if quality_filter and not filtered_contigs_found else "raw")
        logger.info(f"  Final contigs file: {final_contigs_file} ({file_size_mb:.2f} MB, {status}, {filter_status})")
        return final_contigs_file
    else:
...
def run_stage1_independently(args):
    ...
    if not has_reads:
        logger.info("No reads files provided, using contigs file directly (skip assembly and quality assessment)")
        
        if not args.contigs_file or not os.path.exists(args.contigs_file):
            logger.warning("No reads files and no contigs file provided")
            return {"status": "warning", "warning": "Please provide either reads files (--hifi/--ont/--clr) or contigs file (-c/--contigs)"}
        
        final_contigs_file = args.contigs_file
        file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
        
        logger.info(f"Using contigs file directly: {os.path.basename(final_contigs_file)}")
        logger.info(f"  Size: {file_size_mb:.2f} MB")
        logger.info(f"  Assembly: SKIPPED (no reads files)")
        logger.info(f"  Quality assessment: SKIPPED (no reads files)")
        
        merged_dir = os.path.join(args.output_dir, "merged_contigs")
        os.makedirs(merged_dir, exist_ok=True)
        
        final_contigs_copy = os.path.join(merged_dir, "final_contigs.fasta")
        shutil.copy2(final_contigs_file, final_contigs_copy)
        
        result = {
            "status": "success",
            "output_file": args.query_fasta,
            "output_dir": os.path.join(args.output_dir, "assemble_fill"),
            "summary": {
                "assembly_performed": False,
                "reason": "No reads files provided",
                "contigs_source": "user_provided"
            },
            "contigs_info": {
                "generated_contigs": [],
                "external_contigs": [args.contigs_file],
                "final_contigs_file": final_contigs_copy
            },
            "assembly_tools_used": [],
            "final_contigs_file": final_contigs_copy,
        ...
        logger.info(f"Stage 1 (AssembleFill) completed (no assembly, contigs used directly)")
        logger.info(f"   Contigs file: {final_contigs_copy}")
        return result
    ...
    if not has_data:
        ...
        return {"status": "warning", "warning": "Please provide at least one data file (--hifi/--ont/--clr/-c)"}
    
    if MODULES.get('run_assembly_gap_fill') is None:
        warning = "assembly_gap_fill module not available"
    ...
    config_dict = {
        ...
        'quality_min_length': getattr(args, 'quality_min_length', 100000),
        'quality_min_qv': getattr(args, 'quality_min_qv', 30.0),
        'skip_craq': getattr(args, 'skip_craq', False)
    }
    ...
    if config_dict.get('quality_filter', True):
        logger.info(f"Quality filter: ENABLED (min_length={config_dict.get('quality_min_length', 100000):,}bp, "
                   f"min_qv={config_dict.get('quality_min_qv', 30.0):.1f})")
    else:
        logger.info("Quality filter: DISABLED")
    ...
    try:
        ...
        all_reads_files = []
        if args.hifi:
            all_reads_files.extend([f for f in args.hifi if os.path.exists(f)])
        if args.ont:
            all_reads_files.extend([f for f in args.ont if os.path.exists(f)])
        if args.clr:
            all_reads_files.extend([f for f in args.clr if os.path.exists(f)])
        ...
        final_contigs_file = prepare_final_contigs_files(
            ...
            args.output_dir, 
            logger,
            assembly_output_dir=args.output_dir,
            quality_filter=getattr(args, 'quality_filter', True),
            quality_min_length=getattr(args, 'quality_min_length', 100000),
            quality_min_qv=getattr(args, 'quality_min_qv', 30.0),
            skip_craq=getattr(args, 'skip_craq', False),
            reads_files=all_reads_files,
            skip_reassessment=False,
            skip_assembly=not has_reads
        )
        ...
        result = {
            ...
            "final_contigs_file": final_contigs_file,
            "quality_filter_applied": config_dict.get('quality_filter', True)
        }
        ...
        if final_contigs_file:
            file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
            logger.info(f"   Final contigs file: {final_contigs_file} ({file_size_mb:.2f} MB)")
...
def run_stage2_independently(args):
    ...
    if not final_contigs_file:
        logger.info("Searching for quality-filtered contigs files...")
        filtered_file = find_quality_filtered_contigs_in_directory(args.output_dir, logger)
        if filtered_file:
            final_contigs_file = filtered_file
            file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024)
            logger.info(f"Found quality-filtered contigs file: {final_contigs_file}")
            logger.info(f"  Size: {file_size_mb:.2f} MB")
    ...
    try:
        ...
        logger.info(f"   Contigs used: {os.path.basename(final_contigs_file)}")
...
def run_stage3_independently(args):
    ...
    logger.info(f"Using contigs: {os.path.basename(final_contigs_file)}")
...
def run_final_fill_independently(args, logger, input_genome: str, contigs_file: Optional[str] = None):
    ...
    if contigs_file and os.path.exists(contigs_file):
        ...
        logger.info(f"Final refill using contigs file: {os.path.basename(contigs_file)}")
    else:
    ...
    logger.info("Quality filter: DISABLED (contigs already filtered in Stage 1)")
    ...
    try:
        ...
        logger.info(f"   Quality filter: Disabled (contigs already filtered)")
        return result
...
class GenomeRepairController:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.quality_filter = config.get('quality_filter', True)
        self.quality_min_length = config.get('quality_min_length', 100000)
        self.quality_min_qv = config.get('quality_min_qv', 30.0)
        self.skip_craq = config.get('skip_craq', False)
        
        os.makedirs(self.work_dir, exist_ok=True)
        
        self.logger.info("=" * 80)
        self.logger.info("Genome Repair Tools v2.5 with Enhanced Quality Filtering")
        self.logger.info("=" * 80)
        
        if self.quality_filter:
            self.logger.info(f"Quality filter: ENABLED (min_length={self.quality_min_length:,}bp, min_qv={self.quality_min_qv:.1f})")
            if self.skip_craq:
                self.logger.info("  CRAQ assessment: SKIPPED (faster processing)")
        else:
            self.logger.info("Quality filter: DISABLED")
    ...
    def find_quality_filtered_contigs_in_workdir(self) -> Optional[str]:
        return find_quality_filtered_contigs_in_directory(self.work_dir, self.logger)
    
    def prepare_final_contigs_file(self, generated_contigs: List[str], skip_reassessment: bool = False, skip_assembly: bool = False) -> Optional[str]:
        if skip_assembly and self.external_contigs:
            merged_dir = os.path.join(self.work_dir, "merged_contigs")
            os.makedirs(merged_dir, exist_ok=True)
            
            final_contigs_file = os.path.join(merged_dir, "final_contigs.fasta")
            
            if self.external_contigs and os.path.exists(self.external_contigs[0]):
                shutil.copy2(self.external_contigs[0], final_contigs_file)
                file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024)
                self.logger.info(f"Using external contigs file (no assembly): {os.path.basename(self.external_contigs[0])}")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
                self.logger.info(f"  Quality assessment: SKIPPED (no reads files)")
        ...
        if skip_reassessment and os.path.exists(final_contigs_file):
            ...
            if file_size > 1000:
                file_size_mb = file_size / (1024 * 1024)
                self.logger.info(f"Using existing final_contigs.fasta (skip reassessment)")
                self.logger.info(f"  File: {final_contigs_file}")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
                self.logger.info(f"  Note: Quality assessment skipped as requested")
                self.final_contigs_file = final_contigs_file
                return final_contigs_file
            else:
                self.logger.warning(f"  Existing final_contigs.fasta is too small ({file_size} bytes), will recreate")
        
        if self.quality_filter:
            self.logger.info("Searching for pre-existing quality-filtered contigs...")
            
            filtered_file = self.find_quality_filtered_contigs_in_workdir()
            
            if not filtered_file:
                stage1_dir = os.path.join(self.work_dir, "assemble_fill")
                if os.path.exists(stage1_dir):
                    filtered_file = find_quality_filtered_contigs_in_directory(stage1_dir, self.logger)
            
            if filtered_file:
                shutil.copy2(filtered_file, final_contigs_file)
                file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024)
                self.logger.info(f"Using existing quality-filtered contigs file: {os.path.basename(filtered_file)}")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
                self.logger.info(f"  Source: {filtered_file}")
                self.logger.info(f"  (Skipping re-merge and re-filter)")
                self.final_contigs_file = final_contigs_file
                self.quality_filter_already_applied = True
                return final_contigs_file
        
        raw_merged_file = os.path.join(merged_dir, "raw_merged_contigs.fasta")
        
        if len(all_contigs) == 0:
            self.logger.warning("No contigs files available, creating empty final_contigs.fasta")
            with open(final_contigs_file, 'w') as f:
                f.write("# No contigs available - created as placeholder\n")
            ...
            shutil.copy2(all_contigs[0], raw_merged_file)
            self.logger.info(f"  Single contigs file: {os.path.basename(all_contigs[0])}")
        else:
        ...
        if self.quality_filter and not self.quality_filter_already_applied and os.path.exists(final_contigs_file) and os.path.getsize(final_contigs_file) > 100:
            self.logger.info("Applying quality assessment and filtering to merged contigs...")
            
            if MODULES.get('run_assembly_gap_fill') is None:
                self.logger.warning("  assembly_gap_fill module not available, skipping quality filter")
            else:
                try:
                    shutil.copy2(final_contigs_file, final_contigs_file + '.bak')
                    
                    reads_files = []
                    if 'hifi_files' in self.config:
                        reads_files.extend(self.config.get('hifi_files', []))
                    if 'ont_files' in self.config:
                        reads_files.extend(self.config.get('ont_files', []))
                    if 'clr_files' in self.config:
                        reads_files.extend(self.config.get('clr_files', []))
                    ...
                    config_dict = {
                        'query_fasta': final_contigs_file,
                        'output_prefix': quality_filter_prefix,
                        'threads': min(self.config.get('threads', 16), 32),
                        'verbose': self.config.get('verbose', True),
                        'use_tgs': False,
                        'assembly_tools': None,
                        'skip_assembly': True,
                        'quality_filter': True,
                        'quality_min_length': self.quality_min_length,
                        'quality_min_qv': self.quality_min_qv,
                        'skip_craq': self.skip_craq
                    }
                    ...
                    self.logger.info(f"  Running quality filter with min_length={self.quality_min_length:,}bp, min_qv={self.quality_min_qv:.1f}")
                    ...
                    if fill_result.get('status') == 'success' and fill_result.get('output_file'):
                        ...
                        if os.path.exists(filtered_file):
                            ...
                            if raw_size > 0:
                                ...
                                self.logger.info(f"  Quality filter applied successfully: {reduction:.1f}% reduction")
                            else:
                                self.logger.info(f"  Quality filter applied successfully")
                            
                            self.quality_filter_already_applied = True
                        else:
                            self.logger.warning("  Filtered file not found, using raw merged file")
                            if os.path.exists(final_contigs_file + '.bak'):
                                shutil.copy2(final_contigs_file + '.bak', final_contigs_file)
                    else:
                        self.logger.warning(f"  Quality filter failed: {fill_result.get('message', 'Unknown error')}")
                        self.logger.warning("  Using raw merged file (no quality filter)")
                        if os.path.exists(final_contigs_file + '.bak'):
                            shutil.copy2(final_contigs_file + '.bak', final_contigs_file)
        ...
        if os.path.exists(final_contigs_file) and os.path.getsize(final_contigs_file) > 0:
            ...
            filter_status = "pre-filtered" if self.quality_filter_already_applied else "raw"
            self.logger.info(f"  Final contigs file: {final_contigs_file} ({file_size_mb:.2f} MB, {status}, {filter_status})")
            self.final_contigs_file = final_contigs_file
            return final_contigs_file
        else:
    ...
    def get_contigs_for_stage(self, stage_num: int) -> Optional[str]:
        ...
        if self.final_contigs_file and os.path.exists(self.final_contigs_file):
            ...
            if file_size > 100:
                file_size_mb = file_size / (1024 * 1024)
                self.logger.info(f"Stage {stage_num}: Using merged contigs file (highest quality): {os.path.basename(self.final_contigs_file)}")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
        ...
        if self.quality_filter:
            self.logger.info(f"Stage {stage_num}: Searching for quality-filtered contigs...")
            filtered_file = self.find_quality_filtered_contigs_in_workdir()
            if filtered_file:
                file_size_mb = os.path.getsize(filtered_file) / (1024 * 1024)
                self.logger.info(f"Stage {stage_num}: Found quality-filtered contigs file")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
                return filtered_file
    
        if hasattr(self, 'user_contigs_file') and self.user_contigs_file and os.path.exists(self.user_contigs_file):
            file_size = os.path.getsize(self.user_contigs_file)
            if file_size > 100:
                file_size_mb = file_size / (1024 * 1024)
                self.logger.info(f"Stage {stage_num}: Using user-provided contigs file (fallback): {os.path.basename(self.user_contigs_file)}")
                self.logger.info(f"  Size: {file_size_mb:.2f} MB")
    ...
    def _run_assemble_fill(self, input_file: str) -> StageResult:
        ...
        if not has_reads:
            self.logger.info("No reads files provided, skipping assembly and using external contigs directly")
            
            if not self.external_contigs:
                self.logger.warning("No external contigs available, stage 1 will have no contigs output")
                result.success = True
                result.output_files = {'output_genome': input_file}
                result.statistics = {'skipped': True, 'reason': 'No reads and no contigs'}
                return result
            
            self.logger.info("Preparing final contigs file (skip assembly)...")
            final_contigs_file = self.prepare_final_contigs_file([], skip_reassessment=False, skip_assembly=True)
            ...
            self.logger.info(f"Stage 1 completed (no assembly, using external contigs)")
            if final_contigs_file:
                file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
                self.logger.info(f"  Contigs file: {final_contigs_file} ({file_size_mb:.2f} MB)")
        ...
        if self.quality_filter:
            self.logger.info(f"Stage 1 quality filter: ENABLED (min_length={self.quality_min_length:,}bp, min_qv={self.quality_min_qv:.1f})")
        else:
            self.logger.info("Stage 1 quality filter: DISABLED")
        ...
        try:
            ...
            self.logger.info("Creating final contigs file with quality filtering...")
            final_contigs_file = self.prepare_final_contigs_file(generated_contigs, skip_reassessment=False)
            ...
            summary['quality_filter_applied'] = self.quality_filter
            ...
            if final_contigs_file:
                file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
                self.logger.info(f"Final contigs file created: {final_contigs_file} ({file_size_mb:.2f} MB)")
    ...
    def _run_patch_repair(self, input_file: str) -> StageResult:
        ...
        self.logger.info(f"Contigs file: {os.path.basename(contigs_file)}")
    ...
    def _run_correct_refill(self, input_file: str) -> StageResult:
        ...
        self.logger.info(f"Using contigs: {os.path.basename(contigs_file)}")
        ...
        self.logger.info("Note: Contigs already quality-filtered in Stage 1, skipping re-assessment")
    ...
    def _run_final_fill_integrated(self, input_file: str, result: StageResult, 
        ...
        self.logger.info("Quality filter: DISABLED (contigs already filtered in Stage 1)")
        ...
        try:
            ...
            self.logger.info(f"Final refill completed (contigs only, quality filter disabled)")
    ...
    def _run_telomere_recover(self, input_file: str) -> StageResult:
        ...
        try:
            ...
            self.logger.warning(f"Stage 4 execution exception: {str(e)}, using original file")
            result.success = True
            result.output_files = {'output_genome': input_file}
            result.statistics = {
    ...
    def run_pipeline(self, stages_to_run: List[int] = None) -> Dict[str, Any]:
        ...
        if self.quality_filter:
            self.logger.info(f"Quality filter: ENABLED (min_length={self.quality_min_length:,}bp, min_qv={self.quality_min_qv:.1f})")
            if self.skip_craq:
                self.logger.info("  CRAQ assessment: SKIPPED")
        else:
            self.logger.info("Quality filter: DISABLED")
    ...
    def _generate_report(self, total_time: float) -> Dict[str, Any]:
        report = {
            ...
            "contigs_info": {
                ...
                "quality_filter": self.quality_filter,
                "quality_filter_already_applied": self.quality_filter_already_applied,
                "quality_min_length": self.quality_min_length,
                "quality_min_qv": self.quality_min_qv,
                "skip_craq": self.skip_craq
            },
    ...
    def _print_summary(self, report: Dict[str, Any]):
        ...
        quality_filter = contigs_info.get('quality_filter', True)
        quality_filter_applied = contigs_info.get('quality_filter_already_applied', False)
        
        self.logger.info(f"Contigs sources: {external_count} external files")
        if final_contigs_file:
            file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
            self.logger.info(f"Final contigs file: {os.path.basename(final_contigs_file)} ({file_size_mb:.2f} MB)")
        
        if quality_filter:
            min_length = contigs_info.get('quality_min_length', 100000)
            min_qv = contigs_info.get('quality_min_qv', 30.0)
            skip_craq = contigs_info.get('skip_craq', False)
            self.logger.info(f"Quality filter: ENABLED (min_length={min_length:,}bp, min_qv={min_qv:.1f})")
            self.logger.info(f"  Applied: {'Yes' if quality_filter_applied else 'No'}")
            if skip_craq:
                self.logger.info("  CRAQ assessment: SKIPPED")
        else:
            self.logger.info("Quality filter: DISABLED")
        ...
        for stage_name, result in report.get('stage_results', {}).items():
            ...
            if stage_name == 'correct_refill' and result.get('success'):
                ...
                if 'fill_stats' in stats:
                    ...
                    quality_filter = stats.get('quality_filter', False)
                    ...
                    self.logger.info(f"    - Quality filter: {'Enabled' if quality_filter else 'Disabled (already filtered in Stage 1)'}")
            else:
                stats = result.get('statistics', {})
                contigs_info_str = ""
        ...
        if final_stage:
            final_output = report['stage_results'][final_stage].get('output_files', {}).get('output_genome')
            if final_output:
                self.logger.info(f"\nFinal output file: {os.path.basename(final_output)} (with original chromosome names)")

def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Genome Repair Tools v2.5 with Enhanced Quality Filtering",
    ...
    parser.add_argument('--quality-filter', action='store_true', default=True,
                       help='Enable quality assessment and filtering for contigs (default: True)')
    parser.add_argument('--no-quality-filter', action='store_false', dest='quality_filter',
                       help='Disable quality assessment and filtering for contigs')
    ...
    parser.add_argument('--skip-craq', action='store_true', default=False,
                       help='Skip CRAQ assessment for faster processing (default: False)')
...
def main():
    ...
    print(f"\n{'='*80}")
    print("Genome Repair Tools v2.5 with Enhanced Quality Filtering")
    print(f"{'='*80}")
    ...
    if quality_filter:
        print(f"  Quality filter: ENABLED (min_length={quality_min_length:,}bp, min_qv={quality_min_qv:.1f})")
        if skip_craq:
            print(f"    CRAQ assessment: SKIPPED")
    else:
    ...
    if quality_filter:
        print(f"    - Min length: {quality_min_length:,}bp")
        print(f"    - Min QV: {quality_min_qv:.1f}")
        print(f"    - Skip CRAQ: {'Yes' if skip_craq else 'No'}")
    if assembly_tools:
        print(f"  Assembly tools: {', '.join(assembly_tools)} (Stage 1 only)")
    else:
        print(f"  Assembly tools: All available tools (Stage 1 only)")
    print(f"  Contigs management: Enhanced (auto-find quality-filtered files)")
    print(f"  Stage 3 mode: Contigs only (no re-assembly, quality filter disabled)")
    ...
    if specified_stages:
        print(f"dependent Execution Mode:")
        results = []
        ...
        if results:
            ...
            if 'final_contigs_file' in last_result:
                file_size_mb = os.path.getsize(last_result['final_contigs_file']) / (1024 * 1024) if os.path.exists(last_result['final_contigs_file']) else 0
                print(f"  Final contigs file: {last_result['final_contigs_file']} ({file_size_mb:.2f} MB)")
            ...
            if 'quality_filter' in last_result:
                print(f"  Quality filter in last stage: {'Enabled' if last_result.get('quality_filter') else 'Disabled (already filtered)'}")
            sys.exit(0)
        ...
        try:
            ...
            if result.get('status') == 'success':
                ...
                if final_contigs_file:
                    file_size_mb = os.path.getsize(final_contigs_file) / (1024 * 1024) if os.path.exists(final_contigs_file) else 0
                    print(f"  Final contigs file: {os.path.basename(final_contigs_file)} ({file_size_mb:.2f} MB)")
                
                if quality_filter:
                    min_length = contigs_info.get('quality_min_length', 100000)
                    min_qv = contigs_info.get('quality_min_qv', 30.0)
                    print(f"  Quality filter: ENABLED (min_length={min_length:,}bp, min_qv={min_qv:.1f})")
                    print(f"  Applied: {'Yes' if quality_filter_applied else 'No'}")
                else:
                    print(f"  Quality filter: DISABLED")
                ...
                if 'correct_refill' in result.get('stage_results', {}):
                    ...
                    if 'quality_filter' in stage3_stats:
                        print(f"  Stage 3 quality filter: {'Enabled' if stage3_stats['quality_filter'] else 'Disabled (already filtered in Stage 1)'}")
